  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md transition">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {showRestaurant && job.restaurant && (
              <span className="text-sm text-gray-500">{job.restaurant.business_name} - {job.restaurant.city}</span>
            )}
          </div>
        </div>
      ...
      {job.experience_required > 0 && (
        <p className="text-xs text-gray-500 mb-2">Min. {job.experience_required} years experience required</p>
      )}
      ...
      {job.assignments && job.assignments.length > 0 && (
        <div className="border-t border-gray-100 pt-3 mb-3">
          <p className="text-xs font-medium text-gray-500 mb-1">Assigned Provider(s):</p>
          {job.assignments.map(a => (
            <div key={a.id} className="flex items-center justify-between text-sm">
              <span className="text-gray-700">{a.chef?.user?.full_name || 'Unknown'}</span>
              <span className={`text-xs font-medium ${
                a.status === 'accepted' ? 'text-green-600' :
                a.status === 'declined' ? 'text-red-600' :
                a.status === 'pending' ? 'text-yellow-600' : 'text-gray-500'
              }`}>{a.status}</span>
            </div>
          ))}
        </div>
