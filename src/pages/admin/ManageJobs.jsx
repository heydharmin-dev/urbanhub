          ...
          {filteredJobs.map(job => (
            <JobCard key={job.id} job={job} showRestaurant actions={
              <div className="flex gap-2 w-full">
                {job.status === 'open' && (
                  <Button onClick={() => openAssignModal(job)} size="sm">
                    Assign Provider
                  </Button>
                )}
                {job.status === 'matched' && (
                  <Button onClick={() => handleUpdateStatus(job.id, 'filled')} size="sm"
                    className="bg-[#0a0a0a] hover:bg-[#1a1a1a] text-white">
                    Mark Filled
                  </Button>
                )}
                {(job.status === 'open' || job.status === 'matched') && (
                  <Button variant="ghost" size="sm" onClick={() => handleUpdateStatus(job.id, 'closed')}
          ...
          <div className="space-y-4">
            <div className="bg-primary/10 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="font-semibold text-foreground">{assignModal.title}</h3>
                <ServiceCategoryBadge categoryId={assignModal.service_category} size="xs" />
              </div>
              <p className="text-sm text-muted-foreground">{assignModal.restaurant?.business_name} - {assignModal.location}</p>
            ...
            <div>
              <Label className="mb-1">Note for Provider (optional)</Label>
              <Textarea value={adminNotes} onChange={(e) => setAdminNotes(e.target.value)} rows={2}
                className="resize-none" placeholder="Add a note..." />
            </div>
            ...
            <div className="max-h-80 overflow-y-auto space-y-2">
              {providersLoading ? (
                ...
                <p className="text-center text-muted-foreground py-6 text-sm">No matching providers</p>
              ) : (
                filteredProviders.map(p => (
                  <div key={p.id} className="flex items-center justify-between border border-border rounded-lg p-3 hover:bg-muted/50 transition">
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="font-medium text-foreground">{p.user?.full_name}</p>
                    ...
                    <Button onClick={() => handleAssign(p.id)} disabled={assigningId === p.id} size="sm">
